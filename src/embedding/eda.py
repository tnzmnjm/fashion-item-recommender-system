...
df = pd.read_csv(os.path.join('data', 'styles.csv'),
                 sep=',',
...
vectorised_matrix = np.load(os.path.join('models', 'features_fullx2048.npy'))
ids = np.load(os.path.join('models', 'ids_full.npy'))
...
cloud.to_file(os.path.join('docs', 'images', 'wordcloud.png'))
...
image_folder = Path(os.path.join('data', 'images') + os.path.sep)
...
# I need to check the opposite side as well. How many photos exist which there is no metadata available for them?
ids_skipped = []
skipped_matrix_rows = []
for i in range(vectorised_matrix.shape[0]):
    if len(df.loc[df.id == ids[i]]) == 0:
        ids_skipped.append(ids[i])
        skipped_matrix_rows.append(i)
        continue

print(f'There are {len(skipped_matrix_rows)} photos for which there is no metadata available.')

X = np.delete(vectorised_matrix, skipped_matrix_rows, axis=0)

mask = np.isin(ids, ids_skipped)
ids_cleaned = ids[~mask]

np.save(os.path.join('models', 'vectorised_matrix_cleaned'), X)
np.save(os.path.join('models', 'ids_cleaned'), ids_cleaned)
